...
import { OrderItemList } from "./dnd/order-items";
...
const ListItems = ({ members, board, lists }: ListItemsProps) => {
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        if (active.data.current?.type === "list") {
            const overId = findContainer(String(over.id)) || String(over.id).replace("list:", "")
            if (active.id === overId) return
            const oldIndex = realtimeLists.findIndex(l => l.id === active.id)
            const newIndex = realtimeLists.findIndex(l => l.id === overId)
