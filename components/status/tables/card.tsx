...
const ListCard = ({
    ...
    const { isOver, setNodeRef } = useDroppable({
        id: `list:${list.id}`,
        disabled: active?.data.current?.type === "list" || activeCardListId === list.id,
        data: {
            type: "list",
            listId: list.id
        }
    ...
    return (
        <>
            <div
                className="w-full rounded-lg border border-input bg-muted/10 transition-colors overflow-hidden"
            >
                <div
                    {...dragHandleProps}
                ...
                >
                    {realtimeCards.length === 0 ?
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-border text-xs text-muted-foreground bg-muted/5">
                                    <th className="py-2 px-4 font-medium">Task</th>
                                    <th className="py-2 px-4 font-medium">Assignees</th>
